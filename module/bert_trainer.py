...
def run_epoch(dataset,
    ...
    for batch in tqdm(dataset, total=num_batches):
        ...
        if len(targets) > batch_size:
            target_encodings = []
            for batch_idx in trange(len(targets) // batch_size + 1):
                start_idx = batch_idx * batch_size
                end_idx = (batch_idx + 1) * batch_size
                batch = targets[start_idx:end_idx]
                if len(batch) == 0:
                    break
                target_embeddings, target_lengths = batch_embedder.embed(batch)
                # target_encoding = model.encode(target_embeddings, target_lengths)
                target_encoding = model.encode_class(target_embeddings, target_lengths)
                target_encodings.extend(target_encoding)
                del target_encoding
        ...
        if config['loss_type'] =='cross_entropy':
            # Reshape scores to line up positive and negative scores for a given query on a row
        ...
        if train:
            ...
            loss.backward()
            if iter_count % config['gradient_accumulation_steps'] == 0:
                optimizer.step()
                scheduler.step()  # Update learning rate schedule
                model.zero_grad()
                
            # Print/write train results
            if iter_count % (config['print_frequency']*config['gradient_accumulation_steps']) == 0:
                # Compute statistics
