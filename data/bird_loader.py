...
home = str(Path.home())
datadir = '../interactive/CUB_data/'

def load_data(config, preprocessor):
    
    if config['use_proto']:
        data_records, clsatt, _ = read_protodata(config)
    else:
        data_records , clsatt = read_data(config)
    data_train, data_val = split_data(data_records, r =0.2)
    if config['debug']==True:
        data_train, data_val = data_train[:1000], data_val[:200]
    train_queries, train_targets= parse_traindata( data_train, clsatt, config=config, includetag=config['includetag'])

    print('There are {} training examples'.format(len(train_queries)))
    print('There are {} training faqs'.format(len(data_train)))
    print('There are {} validation faqs'.format(len(data_val)))

    idx = np.random.choice(len(data_train))
    print('\nQUERY example: {} \nTARGET example: {}\n'.format( train_queries[idx], train_targets[idx] ))
    ...
    train_data = DataIterator(train_queries,
                                    ...
                                    negativepool = list(set(train_targets)),
    ...
    print('\nVocabulary size of training set= {:,}'.format(len(word_to_index)))

    # ================================ test dataset ================================
    val_datalist = defaultdict()
    tagrange =[0, 1]
    for initq in [1, 0]:
        for tag_r in tagrange:
            ...
            val_key = '_'+str(initq)+'_'+str(tag_r)
            val_queries, val_targets = parse_valdata( data_val, clsatt , initq, tag_r, config=config, includetag=config['includetag'])
            print('\nBuilding validation dataset, {}'.format(val_key))
            val_data = DataIterator(val_queries,
                                          ...
                                          reuse_negatives=True,
                                          negativepool = list(set(np.concatenate((train_targets, val_targets)))),
            ...
            val_datalist[val_key] = val_data

    val_data = val_datalist['_1_0']
    test_datalist = val_datalist
